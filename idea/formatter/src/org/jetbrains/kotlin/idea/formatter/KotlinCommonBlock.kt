...
abstract class KotlinCommonBlock(
    ...
    private fun splitSubBlocksOnDot(nodeSubBlocks: List<ASTBlock>): List<ASTBlock> {
        ...
        val operationBlockIndex = nodeSubBlocks.indexOfBlockWithType(QUALIFIED_OPERATION).let { if (it == -1) return nodeSubBlocks else it }
    ...
    private fun ASTBlock.processBlock(indent: Indent?, wrap: Wrap?): ASTBlock {
        ...
        val resultIndent = indent ?: createIndentForQualifiedExpression(this)

        val newBlock = subBlocks.elementAt(index).processBlock(resultIndent, resultWrap)
        return subBlocks.replaceBlock(newBlock, index).let {
            val operationIndex = subBlocks.indexOfBlockWithType(QUALIFIED_OPERATION)
            if (operationIndex != -1)
                it.splitAtIndex(operationIndex, resultIndent, resultWrap)
            else
                it
